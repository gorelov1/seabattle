...
import { AccountManager, ICON_LIBRARY, } from "../accountManager.js";
...
function createTestApp() {
    const accountManager = new AccountManager(createInMemoryAccountQueries(), createInMemoryAuthTokenQueries(), TEST_JWT_SECRET);
    const app = express();
...
describe("POST /accounts/login", () => {
    ...
    it("JWT payload contains accountId and tokenId", async () => {
        const res = await request(app)
            ...
            .send({ email: validRegistration.email, password: validRegistration.password });
        expect(res.status).toBe(200);
        const decoded = jwt.verify(res.body.jwt, TEST_JWT_SECRET);
