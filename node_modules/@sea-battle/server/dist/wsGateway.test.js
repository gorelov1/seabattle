...
import { Column, Orientation, ShipType, TurnPhase, createEmptyBoard, placeShip, } from "@sea-battle/domain";
 ...
 */
function buildFullBoard(ownerId) {
    const placements = [
...
// ---------------------------------------------------------------------------
function createShootingSession(sessionService, playerA, playerB) {
 ...
 */
function connectWsWithFirstMessage(url) {
    const ws = new WebSocket(url);
...
describe("WsGateway integration tests", () => {
    ...
    // -------------------------------------------------------------------------
    function wsUrl(sessionId, playerId) {
    ...
    it("does not send OpponentReconnected after timeout has expired", async () => {
        ...
        const unexpectedBobMessage = new Promise((resolve) => {
    ...
    it("does not broadcast events when non-active player fires a shot", async () => {
        ...
        const unexpectedAlice = new Promise((resolve) => {
        ...
        const unexpectedBob = new Promise((resolve) => {
            const timer = setTimeout(() => resolve(null), 100);
            wsB.once("message", (data) => {
                clearTimeout(timer);
                resolve(JSON.parse(data.toString()));
...
//# sourceMappingURL=wsGateway.test.js.map