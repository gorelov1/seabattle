...
import { GameService, deserializeBoard, serializeBoard } from "./gameService.js";
import { SessionService, type SessionData, type SessionQueries } from "./sessionService.js";
 ...
 */
function buildDeterministicBoard(ownerId: string, shipCount: number = 10): Board {
  const allPlacements = [
  ...
  for (let i = 0; i < shipCount; i++) {
    ...
    if (!r.ok) throw new Error(`Failed to place ship ${i}: ${r.error}`);
    board = r.value;
  }
  return board;
}
 ...
 */
function buildBoardWithEmptyA1(ownerId: string): Board {
  const placements = [
 ...
 */
function buildAlmostSunkBoard(ownerId: string): Board {
  ...
  if (!r.ok) throw new Error(`Failed to place PatrolBoat: ${r.error}`);
  board = r.value;

  // Inject 9 pre-sunk ships (19 total sunk segments)
  const preSunkShips: Ship[] = [
    {
...
function createPlacementSession(
  ...
  if (!result.ok) throw new Error("Failed to join session");
  return result.value;
}
...
function createShootingSessionWithKnownMiss(
  sessionService: SessionService,
  playerA = "alice",
  playerB = "bob"
): SessionData {
...
function createShootingSessionWithKnownHit(
  sessionService: SessionService,
  playerA = "alice",
  playerB = "bob"
): SessionData {
...
function createSessionWithAlmostSunkFleet(
  sessionService: SessionService,
  playerA = "alice",
  playerB = "bob"
): SessionData {
...
describe("GameService.handlePlacement", () => {
  ...
  it("rejected outside Placement phase — Shooting", () => {
    const session = createPlacementSession(sessionService);
    const shootingSession: SessionData = { ...session, status: "Shooting" };
  ...
  it("rejected outside Placement phase — Finished", () => {
    const session = createPlacementSession(sessionService);
    const finishedSession: SessionData = { ...session, status: "Finished" };
...
describe("GameService.handleShot", () => {
  ...
  it("turn switches to bob after alice misses", () => {
    ...
    const newTurnState = JSON.parse(result.value.session.turnState) as { activePlayer: string };
...
describe("Property 21: Turn order enforced — non-active player shots rejected", () => {
  ...
  it("non-active player shot is rejected and board/turn state are unchanged", () => {
    ...
    const columnArb = fc.constantFrom(
      Column.A, Column.B, Column.C, Column.D, Column.E,
      Column.F, Column.G, Column.H, Column.I, Column.J
    );
    const rowArb = fc.integer({ min: 1, max: 10 }) as fc.Arbitrary<Row>;
    ...
    fc.assert(
      fc.property(coordArb, activePlayerArb, (targetCoord, activePlayer) => {
