...
import { MatchmakingService, type MatchmakingQueries } from "./matchmakingService.js";
...
function createInMemoryMatchmakingQueries(): MatchmakingQueries & {
  _queue: MatchmakingQueueRow[];
} {
  const queue: MatchmakingQueueRow[] = [];
  return {
    _queue: queue,
    enqueue(row: MatchmakingQueueRow) {
      queue.push({ ...row });
    },
...
describe("MatchmakingService.enqueue", () => {
  ...
  let store: ReturnType<typeof createInMemoryMatchmakingQueries>;
...
describe("MatchmakingService.dequeue", () => {
  ...
  it("leaves all other players unchanged after dequeue", () => {
    ...
    expect(queue.map((t) => t.playerId)).toEqual(
...
describe("Property 22: Matchmaking dequeue removes only the specified player", () => {
  ...
  it("dequeuing one player removes exactly that player and leaves all others unchanged", () => {
    ...
    fc.assert(
      fc.property(uniquePlayerIdsArb, (playerIds) => {
        // Build a fresh service for each run
        const queue: MatchmakingQueueRow[] = [];
        const store: MatchmakingQueries & { _queue: MatchmakingQueueRow[] } = {
          _queue: queue,
          enqueue(row: MatchmakingQueueRow) {
            queue.push({ ...row });
          },
        ...
        const allOthersPresent = remainingPlayers.every((id) =>
          idsAfter.includes(id)
        );
