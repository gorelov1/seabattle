...
import { Column, Orientation, ShipType, ShotOutcome, TurnPhase, } from "./types.js";
...
function placeFullFleet(game, playerId) {
...
describe("LocalGame — local mode", () => {
    ...
    it("victory is detected when all 20 segments are sunk", () => {
        ...
        for (const cell of bobShipCells) {
            ...
            if (boardBCell && boardBCell.status !== "Unshot")
                continue;
            if (state.activePlayer !== "alice")
                break;
            game.fireShot("alice", { col: cell.col, row: cell.row });
        }
...
describe("LocalGame — AI mode", () => {
    ...
    it("after human Miss, AI takes its turn automatically", () => {
        ...
        const rows = [10, 9, 8, 7, 6];
        for (const col of cols) {
            for (const row of rows) {
                const state = game.getState();
                if (state.phase !== TurnPhase.Shooting)
                    break;
                ...
                if (result.value.outcome === ShotOutcome.Miss) {
                    ...
                    const validState = afterState.phase === TurnPhase.Finished ||
...
//# sourceMappingURL=localGame.test.js.map