...
const PLAYERS: [string, string] = [PLAYER_A, PLAYER_B];

/** Helper: create a TurnState with the given active player in Shooting phase. */
function state(activePlayer: string) {
...
describe("Property 9: Turn transitions follow shot outcome", () => {
  it(
    "active player switches on Miss and stays on Hit/Sunk for any TurnState and outcome sequence — Validates: Requirements 7.1, 7.2, 7.3, 7.4",
    () => {
      ...
      const playerPairArb = fc
        ...
        .map((arr): [string, string] => [arr[0], arr[1]]);
      ...
      fc.assert(
        fc.property(playerPairArb, outcomeSeqArb, (players, outcomes) => {
          ...
          for (const outcome of outcomes) {
            ...
            if (outcome === ShotOutcome.Miss) {
              ...
              if (next.activePlayer !== expectedNext) return false;
            } else {
              // Hit or Sunk — active player must remain the same
              if (next.activePlayer !== prevActive) return false;
            }

            // Phase must be preserved
            if (next.phase !== state.phase) return false;

            // Input state must not be mutated
            if (state.activePlayer !== prevActive) return false;
