...
import { Column, Orientation, ShipType, FLEET_SPEC, } from "./types.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/**
 ...
 */
function sinkShip(board, coords) {
    let current = board;
    for (const coord of coords) {
        const result = processShot(current, coord);
        if (!result.ok)
            throw new Error(`processShot failed: ${result.error}`);
 ...
 */
function buildAllSunkBoard(ownerId) {
...
describe("check (VictoryDetector)", () => {
    ...
    it("returns none when some ships are sunk but total sunk segments < 20", () => {
        ...
        if (!placed.ok)
            throw new Error("placeShip failed");
    ...
    it("returns none when Battleship (4 segments) is sunk but rest are intact", () => {
        ...
        if (!battleship)
            throw new Error("No battleship found");
...
describe("Property 10: Victory declared iff all 20 segments sunk", () => {
     ...
     * Returns the updated board after all shots.
     */
    function sinkShipOnBoard(board, coords) {
        let current = board;
        for (const coord of coords) {
            const result = processShot(current, coord);
            if (!result.ok)
    ...
    it("check returns Some(winner) iff total sunk segments equals exactly 20, None otherwise", () => {
        ...
        fc.assert(fc.property(arbShipsToSink, (numShipsToSink) => {
            ...
            if (sunkSegments === 20) {
                ...
                if (result.value.playerId !== currentBoard.ownerId)
                    return false;
            }
            else {
    ...
    it("check returns None for any board where sunk segments < 20", () => {
    ...
    it("check returns Some(winner) when all 10 ships are sunk (all 20 segments)", () => {
        fc.assert(fc.property(fc.constant(null), (_) => {
            ...
            if (!result.some)
                return false;
            if (result.value.playerId !== "player2")
                return false;
            return true;
        }), { numRuns: 100 });
    });
});
//# sourceMappingURL=victoryDetector.test.js.map