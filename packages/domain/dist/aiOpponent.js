 ...
 * Requirements: 12.2
 */
export function placeFleet(spec) {
 ...
 */
export function chooseShot(opponentBoard) {
    ...
    if (hitCells.length > 0) {
        ...
        for (const group of sortedGroups) {
            const axisLocked = group.length >= 2
                ...
                : null;
            const candidates = (axisLocked !== null && axisLocked.length > 0)
                ? axisLocked
 ...
 */
function groupConnectedHits(hitCells) {
    ...
    for (const start of hitCells) {
        ...
        while (queue.length > 0) {
            ...
            group.push(current);
            const colIdx = COLUMN_ORDER.indexOf(current.col);
            const neighbors = [];
            if (colIdx > 0)
                neighbors.push({ col: COLUMN_ORDER[colIdx - 1], row: current.row });
            if (colIdx < 9)
                neighbors.push({ col: COLUMN_ORDER[colIdx + 1], row: current.row });
            if (current.row > 1)
                neighbors.push({ col: current.col, row: (current.row - 1) });
            if (current.row < 10)
                neighbors.push({ col: current.col, row: (current.row + 1) });
 ...
 */
function getAxisLockedCandidates(hitCells, board) {
    const COLUMN_ORDER = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"];
    ...
    if (allSameRow) {
        ...
        const colIndices = hitCells.map((c) => COLUMN_ORDER.indexOf(c.col));
        const minCol = Math.min(...colIndices);
        const maxCol = Math.max(...colIndices);
        if (minCol > 0) {
            const leftCoord = { col: COLUMN_ORDER[minCol - 1], row };
            const cell = board.cells.get(serialize(leftCoord));
            if (cell?.status === CellStatus.Unshot)
                candidates.push(leftCoord);
        }
        if (maxCol < 9) {
            const rightCoord = { col: COLUMN_ORDER[maxCol + 1], row };
        ...
        if (minRow > 1) {
            const upCoord = { col, row: (minRow - 1) };
            const cell = board.cells.get(serialize(upCoord));
            if (cell?.status === CellStatus.Unshot)
                candidates.push(upCoord);
        }
        if (maxRow < 10) {
            const downCoord = { col, row: (maxRow + 1) };
            const cell = board.cells.get(serialize(downCoord));
            if (cell?.status === CellStatus.Unshot)
                candidates.push(downCoord);
        }
    }
    return candidates;
}
 ...
 */
function getOrthogonalUnshotNeighbors(hitCells, board) {
    const seen = new Set();
    const candidates = [];
...
function orthogonalNeighbors(coord) {
    const COLUMN_ORDER = ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J"];
    const colIdx = COLUMN_ORDER.indexOf(coord.col);
    if (colIdx === -1)
        return [];
    const neighbors = [];
    // Left
    if (colIdx > 0) {
        neighbors.push({ col: COLUMN_ORDER[colIdx - 1], row: coord.row });
    }
    // Right
    if (colIdx < 9) {
        neighbors.push({ col: COLUMN_ORDER[colIdx + 1], row: coord.row });
...
//# sourceMappingURL=aiOpponent.js.map