...
import { FLEET_SPEC, ShipType, CellStatus, Column, } from "./types.js";
import { serialize } from "./coordinateSystem.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/** Returns a board with specific cells set to a given status. */
function setBoardCellStatus(board, coords, status) {
...
function cellsWithStatus(board, status) {
    const result = [];
...
function orthogonalNeighborCoords(coord) {
    const COLS = [
    ...
    const neighbors = [];
    if (colIdx > 0)
        neighbors.push({ col: COLS[colIdx - 1], row: coord.row });
    if (colIdx < 9)
        neighbors.push({ col: COLS[colIdx + 1], row: coord.row });
    if (coord.row > 1)
        neighbors.push({ col: coord.col, row: (coord.row - 1) });
    if (coord.row < 10)
        neighbors.push({ col: coord.col, row: (coord.row + 1) });
    return neighbors;
}
...
describe("AIOpponent.chooseShot", () => {
    ...
    it("target mode: returns an orthogonal neighbor of a Hit cell when available", () => {
        ...
        const validNeighborKeys = new Set(orthogonalNeighborCoords(hitCoord).map(serialize));
    ...
    it("falls back to hunt when all orthogonal neighbors of Hit cells are already shot", () => {
        ...
        const hitCoord = { col: Column.E, row: 5 };
        board = setBoardCellStatus(board, [hitCoord], CellStatus.Hit);
...
describe("Property 19: AI placement satisfies all placement rules", () => {
    ...
    const VALID_COLS = new Set(COLUMN_ORDER);
    // -------------------------------------------------------------------------
     ...
     */
    function satisfiesOrientationInvariant(board) {
    ...
    function satisfiesAdjacencyInvariant(board) {
     ...
     */
    function satisfiesFleetCompositionInvariant(board) {
        if (!board.ready)
            return false;
    ...
    it("for any invocation of placeFleet, the resulting board satisfies the " +
        "orientation invariant (Property 3), adjacency rule (Property 4), and " +
        "fleet composition invariant (Property 5)", () => {
        fc.assert(fc.property(fc.constant(null), // placeFleet uses Math.random internally; no generator input needed
        (_) => {
            const board = placeFleet(FLEET_SPEC);
            // Property 3: orientation invariant
            if (!satisfiesOrientationInvariant(board))
                return false;
            // Property 4: adjacency rule
            if (!satisfiesAdjacencyInvariant(board))
                return false;
            // Property 5: fleet composition
            if (!satisfiesFleetCompositionInvariant(board))
                return false;
            return true;
        }), { numRuns: 100 });
    });
...
describe("Property 20: AI never fires at an already-shot cell", () => {
    ...
    it("for any board state with 0–80 pre-shot cells, chooseShot always returns a coordinate with status Unshot", () => {
        fc.assert(fc.property(preShootCountArb, (preShootCount) => {
...
//# sourceMappingURL=aiOpponent.test.js.map